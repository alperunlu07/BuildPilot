...
export const STEP_DEFINITIONS: Record<StepType, StepDefinition> = {
  ...
  swiftlint: {
    ...
    label: 'SwiftLint',
    description:
      'Run `swiftlint lint` / `--fix` / `analyze` against the project. Style + best-practice checks for Swift sources.',
    color: '#0ea5e9',
    icon: 'CodeXml',
    fields: [
      {
      ...
      {
        name: 'configFile',
        label: 'Config file (.swiftlint.yml)',
        type: 'text',
        placeholder: '.swiftlint.yml',
      },
      {
      ...
      {
        name: 'quiet',
        label: 'Quiet (suppress non-violation output)',
        type: 'select',
        options: ['false', 'true'] as const,
        defaultValue: 'false',
      },
  ...
  swiftFormat: {
    ...
    label: 'swift-format',
    description:
      "Run Apple's `swift-format lint` (validate) or `format -i` (rewrite in place) over the project's Swift sources.",
    color: '#6366f1',
    icon: 'AlignLeft',
    fields: [
      {
      ...
      {
        name: 'configFile',
        label: 'Config file (.swift-format JSON)',
        type: 'text',
        placeholder: '.swift-format',
      },
      {
